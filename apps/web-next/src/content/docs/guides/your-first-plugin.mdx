...
> **New to plugin development?** Start with a frontend-only plugin (`naap-plugin create my-plugin`) and add a backend later. This guide shows the full-stack path for completeness.
...
> **Tip:** For a simpler start without Docker or Prisma, use `--simple`:
> ```bash
> naap-plugin create task-tracker --template full-stack --simple
> ```
...
Add your models to `packages/database/prisma/schema.prisma`. You can do this manually or use the CLI:

```bash
naap-plugin add model TrackerTask title:String description:String status:String priority:String assigneeId:String createdBy:String
```
...
app.get('/api/v1/task-tracker/tasks', async (req, res) => {
  ...
  // API envelope: { success: true, data: { tasks } }
...
export default function App() {
  ...
  async function loadTasks() {
    ...
    // usePluginApi auto-unwraps the { success, data } envelope
    setTasks(res.tasks ?? []);
  }
