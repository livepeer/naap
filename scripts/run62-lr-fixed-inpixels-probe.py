#!/usr/bin/env python3
"""Run 62 — pymthouse signer ``type=fixed`` (+``inPixels``) decisive re-probe on
the clean v0.9.0 live-runner orch ``:8936`` (``liverunner-v09-orch``, VM
``liverunner-staging-1``, IP 136.66.21.17).

Purpose: capture VERBATIM the ``/generate-live-payment`` response the deployed
pymthouse DMZ signer returns for a native single-shot per-cap challenge minted
with ``type:"fixed"`` and ``inPixels:1``. Prior run61 (signer build serving
``/healthz`` Last-Modified Jul-27) returned ``400 {"error":{"message":"invalid
job type"}}`` for ``type=fixed``. This run re-detects any drift and adds the
``inPixels:1`` field required by the fixed path.
...
Flow (native v0.9.0 ``/apps/{runner}/app/generate`` payment-gated single-shot):
  ...
  Stage B — POST ``/generate-live-payment`` to the pymthouse signer with
            ``type:"fixed"`` + ``inPixels:1`` bound to that challenge. DECISIVE.
            Records raw HTTP status + full body + notable headers verbatim.
  Stage C — comparison mints ``type:"byoc"`` / ``type:"lv2v"`` for build drift.
...
print("=" * 74)
print("RUN 62 — pymthouse signer type=fixed (+inPixels) decisive probe on :8936")
...
# ── Stage B/C: mint with each type; type=fixed(+inPixels) is decisive ────────
caps_b64 = base64.b64encode(byoc_capabilities_from_app(CAP).SerializeToString()).decode()


def mint(sig_type: str, extra: dict | None = None) -> dict:
    ...
    try:
        with _post(f"{origin}/generate-live-payment", json.dumps(payload).encode(), hdrs, 60) as r:
            ...
            try:
                ...
                pass
            return {"type": sig_type, "status": r.status, "body": txt,
                    "keys": keys, "headers": _hdrs(r.headers)}
    except urllib.error.HTTPError as e:
        return {"type": sig_type, "status": e.code, "body": e.read().decode(),
                "headers": _hdrs(e.headers)}
    except Exception as e:
        return {"type": sig_type, "status": "ERR", "body": str(e)[:400], "headers": {}}


print("-" * 74)
print("[B mint type=fixed + inPixels:1]  DECISIVE  (payload sent to signer)")
print(f"   request payload keys: {['orchestrator','type','capabilities','inPixels']}")
r_fixed = mint("fixed", {"inPixels": 1})
print(f"   HTTP {r_fixed['status']}")
print(f"   headers: {json.dumps(r_fixed.get('headers', {}))}")
print(f"   body (verbatim): {r_fixed.get('body')}")
if r_fixed.get("keys"):
    print(f"   keys: {r_fixed['keys']}")

print("-" * 74)
print("[B' mint type=fixed, NO inPixels]  (control)")
r_fixed_noip = mint("fixed")
print(f"   HTTP {r_fixed_noip['status']}  body: {r_fixed_noip.get('body')}")
...
for t in ("byoc", "lv2v"):
    r = mint(t)
    matrix[t] = r
    print(f"   type={t:5s} -> HTTP {r['status']}  body: {r.get('body','')[:260]}")

# ── Verdict ─────────────────────────────────────────────────────────────────
print("=" * 74)
st = r_fixed["status"]
b = r_fixed.get("body", "") or ""
if st == 200 and (r_fixed.get("keys")):
    print("VERDICT: type=fixed(+inPixels) ACCEPTED (200 mint) — signer supports RemoteType_Fixed")
    ...
    print(f"VERDICT: type=fixed(+inPixels) -> HTTP {st} (see verbatim body above)")
print("=" * 74)

json.dump({"challenge": ev, "fixed_inpixels": r_fixed, "fixed_noinpixels": r_fixed_noip,
           "matrix": matrix},
          open("/tmp/run62_fixed.json", "w"), indent=1)
