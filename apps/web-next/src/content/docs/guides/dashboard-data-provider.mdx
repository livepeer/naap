  ...
  return createDashboardProvider(eventBus, {
    ...
    kpi: async (args) => {
      ...
      return {
        ...
        orchestratorsObserved: { value: stats.orchCount, delta: stats.orchDelta },
        dailyUsageMins: { value: stats.usageMins, delta: stats.usageDelta },
        dailySessionCount: { value: stats.streams, delta: stats.streamsDelta },
      };
    },
...
| Type | Fields |
|---|---|
| `KPI` | `successRate`, `orchestratorsObserved`, `dailyUsageMins`, `dailySessionCount` (all `MetricDelta!`) |
