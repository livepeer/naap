  ...
  return createDashboardProvider(eventBus, {
    ...
    kpi: async (args) => {
      ...
      return {
        ...
        dailySessionCount: { value: stats.sessionCount, delta: stats.sessionDelta },
        dailyNetworkFeesEth: { value: stats.networkFeesEth, delta: stats.networkFeesDelta },
        timeframeHours: 24,
      };
    },
...
| Type | Fields |
|---|---|
| `KPI` | `successRate`, `orchestratorsOnline`, `dailyUsageMins`, `dailySessionCount`, `dailyNetworkFeesEth` (all `MetricDelta!`); `timeframeHours` (`Int!`); optional `hourlyUsage` / `hourlySessions` |
