...
export const PlanDetailPage: React.FC = () => {
  ...
  if (!plan) {
    return (
      <div className="p-6 max-w-[1400px] mx-auto">
        <button onClick={() => navigate('/plans')} className="flex items-center gap-1 text-sm text-gray-400 hover:text-gray-200 mb-4 transition-colors">
          <ArrowLeft size={16} /> Back to Plans
        </button>
        <div className="text-center py-20">
          <AlertCircle size={36} className="mx-auto text-gray-500 mb-4" />
          <h2 className="text-lg font-semibold text-gray-200 mb-2">Plan Not Found</h2>
          <p className="text-sm text-gray-400">{error || 'The requested plan does not exist.'}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto space-y-5">
      {/* Back + Header */}
      <button onClick={() => navigate('/plans')} className="flex items-center gap-1 text-sm text-gray-400 hover:text-gray-200 transition-colors">
        <ArrowLeft size={16} /> Back to Plans
      </button>
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-100 tracking-tight">{plan.name}</h1>
            {plan.enabled ? (
              ...
              <span className="flex items-center gap-1 text-[11px] text-gray-500 bg-gray-700/30 px-2 py-0.5 rounded-full">
                <PowerOff size={10} /> Disabled
              </span>
            )}
          </div>
          <p className="text-[12px] text-gray-500 font-mono mt-1">{plan.billingPlanId}</p>
        ...
        <div className="flex items-center gap-2">
          ...
          {savedFlash && (
            <span className="px-2 py-0.5 text-[10px] font-medium rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:text-gray-500 text-white text-sm font-medium rounded-lg transition-colors"
          >
      ...
      <div className="panel p-4">
        <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Capabilities</span>
      ...
      <div className="panel p-5 space-y-5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-blue-400 uppercase tracking-wider">Configuration</span>
          <span className="text-[10px] text-gray-500 ml-2">Edit to see how results change</span>
        </div>

        {/* Top N + Sort By row */}
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Top N</label>
              ...
              {TOP_N_OPTIONS.map((n) => (
                <button
                  ...
                  className={`px-3 py-1 text-xs font-medium rounded transition-colors ${
                    draft.topN === n ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-gray-200'
                  }`}
                >
                  {n}
                </button>
          ...
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Sort By</label>
            <select
              value={draft.sortBy ?? 'slaScore'}
              onChange={(e) => setDraft({ sortBy: e.target.value as PlanSortBy })}
              className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-200 text-sm focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500/40 transition-all"
            >
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-purple-500/30 cursor-pointer"
          />
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-semibold text-purple-400 uppercase tracking-wider">SLA Weights</span>
            <span className="text-[10px] text-gray-500 ml-2">Auto-normalize to 100%</span>
          </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Endpoint & Integration</span>
        </div>
      ...
      {/* Meta Footer */}
      {results && (
        <div className="flex items-center gap-4 text-[11px] text-gray-500 border-t border-gray-700/40 pt-3">
...
const ResultsSection: React.FC<{ results: import('../lib/api').PlanResults | null; loading: boolean }> = ({ results, loading }) => {
  ...
  if (capabilities.length === 0) {
    return (
      <div className="text-center py-10 panel">
        <Activity size={28} className="mx-auto text-gray-500 mb-3" />
        <p className="text-sm text-gray-400">No results available yet. Try adjusting the configuration.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {capabilities.map(([capability, rows]) => {
        ...
        return (
          <div key={capability} className="panel overflow-hidden">
            <button
              onClick={() => setCollapsed((p) => ({ ...p, [capability]: !isCollapsed }))}
              className="w-full px-4 py-3 border-b border-gray-700/60 flex items-center justify-between hover:bg-gray-800/40 transition-colors"
            >
      ...
      <table className="w-full text-[13px]">
        <thead>
          <tr className="border-b border-gray-700/60 text-[11px] uppercase tracking-wider">
            ...
            <th className="px-4 py-2.5 text-right text-gray-500 font-semibold">SLA</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr
              ...
              className="border-b border-gray-700/30 hover:bg-gray-800/60 transition-colors"
            >
              ...
              <td className="px-4 py-3 font-mono text-[11px] text-gray-200 max-w-[260px] truncate" title={row.orchUri}>
                {row.orchUri}
              </td>
