 ...
 * (see beforeEach below; playwright.config does not grep-exclude @oauth).
 */
test.describe('OAuth entrypoints @oauth', () => {
  ...
  test.beforeEach(() => {
    test.skip(
      ...
      'Set RUN_OAUTH_E2E=1 to run OAuth redirect checks (documented in apps/web-next/.env.local.example)',
    );
  });
