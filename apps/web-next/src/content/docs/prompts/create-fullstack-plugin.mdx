...
- Backend runs on its own port in local dev (`plugin.json → backend.devPort` is the single source of truth); on Vercel, the shell proxies API calls to /api/v1/[plugin-name]/* via dedicated Next.js route handlers
...
**CRITICAL — Next.js Route Handlers (Vercel/Production):**
- Every plugin API endpoint **must** have a dedicated Next.js route handler at
  `apps/web-next/src/app/api/v1/{plugin-name}/...`
- The catch-all proxy (`/api/v1/[plugin]/[...path]`) returns **501 Not Implemented**
  on Vercel for any unhandled route — it is NOT a passthrough
- When adding a new backend endpoint, always create a matching route handler file
...
const app = express();
const PORT = process.env.PORT || 4XX; // must match plugin.json → backend.devPort
