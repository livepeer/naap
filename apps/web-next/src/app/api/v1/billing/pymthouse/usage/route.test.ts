...
vi.mock('@/lib/pymthouse-client', () => ({
  getPmtHouseServerClient: vi.fn(() => ({ fetchUsageForExternalUser, getUsage })),
...
describe('GET /api/v1/billing/pymthouse/usage', () => {
  ...
  it('scope=me delegates to fetchUsageForExternalUser', async () => {
    ...
    expect(json.data.currentUser.pipelineModels).toHaveLength(1);
  ...
  it('maps upstream PmtHouseError 404 to generic not found', async () => {
    fetchUsageForExternalUser.mockRejectedValue(
      new PmtHouseError('missing', { status: 404, code: 'NOT_FOUND' }),
    );
    const req = new NextRequest('http://localhost/api/v1/billing/pymthouse/usage?scope=me', {
      headers: { cookie: 'naap_auth_token=tok' },
    });
