...
import { getBYOCSummary } from '@/lib/facade';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const cacheKey = `byoc-summary:${start.slice(0, 13)}:${end.slice(0, 13)}`;

  try {
    const { data, cache } = await bffStaleWhileRevalidate(
      cacheKey,
      () => getBYOCSummary({ start, end }),
      'byoc-summary',
    );
    ...
    console.error('[byoc/summary] error:', err);
    return NextResponse.json(
      { error: { code: 'SERVICE_UNAVAILABLE', message: 'BYOC summary is unavailable' } },
      { status: 503 },
    );
  }
}
