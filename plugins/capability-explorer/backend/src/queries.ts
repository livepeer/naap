...
const DEMO_QUERY_TEMPLATES: DemoQueryTemplate[] = [
  {
    slug: 'top-image-gen',
  ...
  {
    slug: 'high-capacity-video',
    name: 'High Capacity Video',
    category: 'i2v',
    sortBy: 'capacity',
    sortOrder: 'desc',
    limit: 10,
  ...
  {
    slug: 'all-capabilities',
    name: 'All Capabilities Overview',
    sortBy: 'name',
    sortOrder: 'asc',
    limit: 50,
  },
...
export async function seedDemoQueries(scope: QueryScope): Promise<{ created: number; total: number }> {
  const existing = await listQueries(scope);
      ...
      await createQuery(
        {
          ...
          sortBy: tpl.sortBy as CreateCapabilityQueryInput['sortBy'],
          sortOrder: tpl.sortOrder as CreateCapabilityQueryInput['sortOrder'],
          limit: tpl.limit,
          minGpuCount: tpl.minGpuCount,
          maxPriceUsd: tpl.maxPriceUsd,
        },
        scope,
      );
      created++;
