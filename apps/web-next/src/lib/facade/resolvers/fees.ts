...
export async function resolveFees(opts: { days?: number }): Promise<DashboardFeesInfo> {
  ...
  return cachedFetch(`facade:fees:${first}`, 15 * 60 * 1000, async () => {
    ...
    return {
      ...
      weeklyVolumeChangeUsd: round2(percentChange(weekForDisplay?.weeklyVolumeUsd ?? 0, weekForDeltaBase?.weeklyVolumeUsd ?? 0)),
      weeklyVolumeChangeEth: round2(percentChange(weekForDisplay?.weeklyVolumeEth ?? 0, weekForDeltaBase?.weeklyVolumeEth ?? 0)),
      dayData,
      weeklyData,
    };
  });
